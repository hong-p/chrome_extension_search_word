...
function createHoverTip() {
    ...
    hoverTipDiv
        .style
        .setProperty("text-align", "none", "center");
    hoverTipDiv
        .style
    ...
    hoverTipDiv
        .style
        .setProperty("width", "auto", "important");
    hoverTipDiv
        .style
        .setProperty("z-index", "1410065406", "important");
    hoverTipDiv
        .style
        .setProperty("vertical-align", "middle", "important");
    hoverTipDiv
        .style
        .setProperty("padding", "1px 1px 1px 1px", "important");
    hoverTipDiv
        .style
        .setProperty("margin", "0px 0px 0px 0px", "important");
    hoverTipDiv
        .style
        .setProperty("align", "absmiddle", "important");
    hoverTipDiv
        .style
        .setProperty("font-size", String(fontSize) + "pt", "important");
    hoverTipDiv
        .style
        .setProperty("line-height", "normal", "important");
    ...
    var backgroundColorValue = "-webkit-gradient(linear, left top, left bottom, from(" +
            backColor1 + "), to(" + backColor2 + "))";
...
function getHoverText(event) {
    ...
    var dLeft = parseInt(
        currDoc.defaultView.getComputedStyle(currDoc.body, null).getPropertyValue("left")
    );
    if (isNaN(dLeft)) 
        dLeft = 0;
    hoverTipDiv
        .style
        .setProperty("left", String((event.pageX - dLeft) / zoom) + "px", "important");
    var dTop = parseInt(
        currDoc.defaultView.getComputedStyle(currDoc.body, null).getPropertyValue("top")
    );
    if (isNaN(dTop)) 
        dTop = 0;
    var top;
    if (boxPosition == "UP") 
        top = (
            event.pageY - parseInt(offsetDistance) - hoverTipDiv.offsetHeight - dTop
        ) / zoom;
    else 
        top = (event.pageY + parseInt(offsetDistance) - dTop) / zoom;
    hoverTipDiv
        .style
        .setProperty("top", String(top) + "px", "important");
    return text
}
function PositionCorrection(elem) {
    ...
    if (currDoc.body.style.zoom) 
        zoom = parseFloat(currDoc.body.style.zoom);
    var aTop = parseInt(
        currDoc.defaultView.getComputedStyle(elem, null).getPropertyValue("top")
    );
    var aHeight = parseInt(
        currDoc.defaultView.getComputedStyle(elem, null).getPropertyValue("height")
    );
    ...
    elem
        .style
        .setProperty("left", String(left / zoom) + "px", "important")
}
    ...
    chrome
        ...
        .sendRequest(request, function (response) {
            if (response.mean != undefined) {
                ...
                hoverTipDiv
                    .style
                    .setProperty("font-size", String(fontSize / zoom) + "pt", "important");
